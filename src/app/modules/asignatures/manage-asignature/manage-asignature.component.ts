...
@Component({
	...
	styleUrls: ['./manage-asignature.component.scss'],
	animations: fuseAnimations
})
export class ManageAsignatureComponent implements OnInit, OnDestroy {
	id: number | null;
		...
		this._asignaturesService
			...
			.subscribe(
				() => {
					...
					this._router.navigate(['/asignaturas/lista', {m: 1}]);
				},
	...
	updateAsignatura(): void {
		...
		this._asignaturesService
			...
			.subscribe(
				() => {
					this.asignaturaForm.enable();
					this._router.navigate(['/asignaturas/lista', {m: 2}]);
				},
				(response: HttpValidationErrorResponse) => {
					// Re-enable the form
					this.asignaturaForm.enable();
